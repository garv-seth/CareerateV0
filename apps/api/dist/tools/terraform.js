...
exports.terraformTools = exports.planTerraformDeployment = exports.validateTerraformConfig = exports.generateTerraformConfig = void 0;
...
exports.generateTerraformConfig = (0, tools_1.tool)(async ({ resourceType, provider, config }) => {
    try {
        ...
        const tempDir = path.join(process.cwd(), "temp", (0, uuid_1.v4)());
    ...
    schema: zod_1.z.object({
        resourceType: zod_1.z.enum(["vpc", "ec2", "rds", "s3", "aks"]),
        provider: zod_1.z.enum(["aws", "azure", "gcp"]),
        config: zod_1.z.record(zod_1.z.any()).describe("Resource-specific configuration")
    })
});
exports.validateTerraformConfig = (0, tools_1.tool)(async ({ configPath }) => {
    try {
    ...
    schema: zod_1.z.object({
        configPath: zod_1.z.string().describe("Path to Terraform configuration file")
    })
});
exports.planTerraformDeployment = (0, tools_1.tool)(async ({ configPath, workspace }) => {
    try {
...
function generateVPCConfig(provider, config) {
    if (provider === "aws") {
        return `
...
function generateAzureVPCConfig(config) {
    return `
...
function generateEC2Config(provider, config) {
    // Implementation for EC2/VM configs
    return "";
}
function generateRDSConfig(provider, config) {
    // Implementation for RDS/Database configs
    return "";
}
function generateS3Config(provider, config) {
    // Implementation for S3/Storage configs
    return "";
}
function generateAKSConfig(provider, config) {
...
exports.terraformTools = [
    exports.generateTerraformConfig,
    exports.validateTerraformConfig,
    exports.planTerraformDeployment
];
